...
size_t Sys_FileRead (uint32_t handle, void *dest, size_t count)
{
	int t = VID_ForceUnlockedAndReturnState ();
	size_t x = fread (dest, 1, count, sys_handles[handle]);
	VID_ForceLockState (t);
	return x;
}

size_t Sys_FileWrite (uint32_t handle, const void *data, size_t count)
{
	int t = VID_ForceUnlockedAndReturnState ();
	size_t x = fwrite (data, 1, count, sys_handles[handle]);
	VID_ForceLockState (t);
	return x;
}
