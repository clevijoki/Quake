...
// so we can run on Win 3.1, where there isn't necessarily Winsock)
    ...
    pWSAStartup = (PWSASTARTUP_FUNC)GetProcAddress(hInst, "WSAStartup");
    pWSACleanup = (PWSACLEANUP_FUNC)GetProcAddress(hInst, "WSACleanup");
    pWSAGetLastError = (PWSAGETLASTERROR_FUNC)GetProcAddress(hInst, "WSAGetLastError");
    psocket = (PSOCKET_FUNC)GetProcAddress(hInst, "socket");
    pioctlsocket = (PIOCTLSOCKET_FUNC)GetProcAddress(hInst, "ioctlsocket");
    psetsockopt = (PSETSOCKOPT_FUNC)GetProcAddress(hInst, "setsockopt");
    precvfrom = (PRECVFROM_FUNC)GetProcAddress(hInst, "recvfrom");
    psendto = (PSENDTO_FUNC)GetProcAddress(hInst, "sendto");
    pclosesocket = (PCLOSESOCKET_FUNC)GetProcAddress(hInst, "closesocket");
    pgethostname = (PGETHOSTNAME_FUNC)GetProcAddress(hInst, "gethostname");
    pgethostbyname = (PGETHOSTBYNAME_FUNC)GetProcAddress(hInst, "gethostbyname");
    pgethostbyaddr = (PGETHOSTBYADDR_FUNC)GetProcAddress(hInst, "gethostbyaddr");
    pgetsockname = (PGETSOCKNAME_FUNC)GetProcAddress(hInst, "getsockname");
...
int WINS_Read (int socket, byte *buf, int len, struct qsockaddr *addr)
{
	...
	if (ret == -1)
	{
		int err_result = pWSAGetLastError();

		if (err_result == WSAEWOULDBLOCK || err_result == WSAECONNREFUSED)
			return 0;
...
int WINS_Write (int socket, const byte *buf, int len, struct qsockaddr *addr)
{
	int ret;
