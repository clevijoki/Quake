...
static string_t PR_TempString()
{
	static string_t result = { INT_MIN };
	
	if (result.value == INT_MIN)
		result = PR_NewString(pr_string_temp);
...
void PF_ftos (void)
{
	...
	G_INT(OFS_RETURN) = PR_TempString().value;
}
...
void PF_etos (void)
{
	...
	G_INT(OFS_RETURN) = PR_TempString();
}
#endif
